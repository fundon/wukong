...
w.build = function (fn) {
  ...
  function processDone(file, files) {
    return function (done) {
      ware.run(file, files, self, function *() {
        file = this.file;
        if (!file) return done();
        yield file.write(file);
        done(null, file);
      });
    };
  }
  ...
  function *build() {
    ...
    // before
    self._useInit(1, self._before);
    files = yield batchFiles(files, self._before);
    // middle
    self._useInit(0, self._middle);
    var newFiles = yield batch(files);
    // after
    self._useInit(1, self._after);
...
w._useInit = function (type, arr) {
  var ware = this.ware;
  ware.clear();
  ware.use(type ? beforeOrAfterOverride : override);
  if (arr) {
