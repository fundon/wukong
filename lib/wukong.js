...
var debug = require('debug')('wukkong');
...
function Wukong(dir) {
  ...
  this.ware = new Ware();
  // override input
  this.ware.use(function *input(next) { this.input = this.input[0]; yield next; });
...
w.read = function *() {
  ...
  var i, l, filename, buffer, file, stats, parsed;
...
w.write = function *(files) {
  ...
  return yield each(files, function *(file, name) {
    var out = path.join(dest, file.name || name);
