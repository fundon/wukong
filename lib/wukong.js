...
function Wukong(dir) {
  ...
  this.use(override);
  // files's middlewares
  this.filesWare = new Ware();
  this.useForFiles(filesOverride);
}
...
w.useForFiles = function (plugin) {
  this.filesWare.use(plugin);
  return this;
};
...
w.build = function (fn) {
  ...
  co(function *(self, src, dest) {
    ...
    self.runForFiles(files, function *run() {
      files = this.files;
...
w.runForFiles = function (files, fn) {
  this.filesWare.run(files, this, fn);
};
