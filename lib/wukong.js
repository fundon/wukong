...
function Wukong(dir) {
  ...
  this.use(override);
  // files's middlewares
  this.batchWare = new Ware();
  this.useBatch(batchOverride);
}
...
w.useBatch = function (plugin) {
  this.batchWare.use(plugin);
  return this;
};

/**
 *  Get or set the batch `metadata` to pass to templates.
 *
...
w.build = function (fn) {
  ...
  co(function *(self, src, dest) {
    ...
    self.runBatch(files, function *run() {
      files = this.files;
...
w.runBatch = function (files, fn) {
  this.batchWare.run(files, this, fn);
};
